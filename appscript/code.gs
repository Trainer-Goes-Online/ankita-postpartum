/**
 * BodyWorx Postpartum CRM — downstream Meta CAPI event firing engine.
 *
 * Bound to the BodyWorx Postpartum CRM Google Sheet. On every USER edit to a
 * trigger column (attended / qualified / sale_closed) this script reads the
 * lead's row, builds a Meta Conversions API event payload with full hashed
 * user_data, POSTs to graph.facebook.com directly, and stamps the row with the
 * resulting event_id + dedup flag. Failures are logged to the hidden `_Errors`
 * tab so the row remains retry-able.
 *
 * Funnel type: WEBINAR / LIVE CHALLENGE. All buyers attend the SAME scheduled
 * 5-Day Postpartum Recovery Challenge, then the sales team manually sells a
 * high-ticket offer post-challenge. Three downstream events:
 *   - LeadShowUp        (paid lead actually attended the live challenge)
 *   - QualifiedLead     (attendee showed intent for a post-challenge sales call)
 ...
 * only the three downstream events. Both systems share the same Meta pixel ID +
 * access token but never talk directly — the Sheet is the only link.
 *
 * Architecture: talks to Meta directly (no backend proxy). All secrets live in
 * Apps Script PropertiesService. Onboard a new client in ~15 min by pasting this
 * file in and setting three script properties.
 *
 * Source-of-truth schema: 36-column row, A..AJ, per appscript/readme.md.
 */
...
const COL = {
  ...
  // --- Webinar lifecycle: manual / Apps Script-written (X..AJ) ---
  ATTENDED:                  24,  // X  → fires LeadShowUp
  SHOWUP_TIME:               25,  // Y
  LEADSHOWUP_CAPI_EVENT_ID:  26,  // Z
  LEADSHOWUP_CAPI_SENT:      27,  // AA
  QUALIFIED:                 28,  // AB → fires QualifiedLead
  QUALIFIED_TIME:            29,  // AC
  QUALIFIED_CAPI_EVENT_ID:   30,  // AD
  QUALIFIED_CAPI_SENT:       31,  // AE
  SALE_CLOSED:               32,  // AF → fires HighTicketPurchase
...
// is what causes the event to fire. valueCol is only used for the HT sale.
const EVENTS = {
  LEAD_SHOWUP: {
...
function fireDownstreamEvent(sheet, row, cfg) {
  ...
  if (result.ok) {
    // Stamp success. Write STRING 'TRUE' (not boolean) so it renders in
    // dropdown cells with data validation list {TRUE, FALSE}. Reading is
    // handled by isTruthy() which accepts either.
...
function buildUserData(rowData) {
  ...
  // This matches the backend Pabbly payload's external_id (also sha256(email))
  // so the downstream events join the tripwire Purchase via the `em` field.
  const emailNorm = stringAt(rowData, COL.EMAIL).trim().toLowerCase();
  if (emailNorm) {
  ...
  // First name.
  const fnNorm = stringAt(rowData, COL.FIRST_NAME).trim().toLowerCase();
  if (fnNorm) out.fn = [sha256Hex(fnNorm)];

  // Last name.
  const lnNorm = stringAt(rowData, COL.LAST_NAME).trim().toLowerCase();
  ...
  const countryNorm = stringAt(rowData, COL.COUNTRY_CODE).trim().toLowerCase();
...
function setupTriggers() {
  ...
  console.log(msg);
  SpreadsheetApp.getActive().toast(msg, 'BodyWorx CRM', 5);
}
...
function replayPendingEvents() {
  ...
  SpreadsheetApp.getActive().toast('Replayed ' + replayed + ' pending event(s)', 'BodyWorx CRM', 5);
}
